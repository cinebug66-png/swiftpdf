...
export function CompressPdfTool() {
  ...
  const originalSize = useMemo(() => (file ? formatFileSize(file.size) : null), [file]);
  const compressedSizeLabel = useMemo(
    () => (compressedSize != null ? formatFileSize(compressedSize) : null),
    [compressedSize],
  );
  const savingsPercent = useMemo(() => {
    if (!file || compressedSize == null || file.size === 0) return null;
    return Math.max(0, Math.round(((file.size - compressedSize) / file.size) * 100));
  }, [compressedSize, file]);

  const selectFile = (nextFile: File | null) => {
    revokeDownloadUrl(downloadUrl);
    setDownloadUrl(null);
    setDownloadName("compressed.pdf");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await compressPdf(file, (job) => {
        ...
        if (runningTask?.operation === "optimize") {
          setProgressNote("Compressing and optimizing your PDF...");
        } else if (runningTask?.operation === "export/url") {
          setProgressNote("Preparing your compressed PDF download...");
        }
      });
      ...
      setStatus("error");
      setProgressNote("Compression failed.");
    }
  };
    ...
    <>
      ...
      {status === "done" && downloadUrl && (
        <div className="mt-6 rounded-2xl glass p-6 text-center shadow-glow">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Your optimized PDF is ready to download.
          </p>
          <div className="mx-auto mt-4 grid max-w-md gap-2 rounded-2xl bg-card/70 p-4 text-left text-sm text-muted-foreground">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="font-medium text-foreground">{originalSize}</span>
            </div>
            <div className="flex items-center justify-between gap-4">
              <span>Compressed size</span>
              <span className="font-medium text-foreground">{compressedSizeLabel ?? "Ready"}</span>
            </div>
            {savingsPercent != null && (
              <div className="flex items-center justify-between gap-4">
                <span>Saved</span>
                <span className="font-medium text-foreground">{savingsPercent}%</span>
              </div>
            )}
          </div>
      ...
      <div className="mt-6 flex items-center justify-center gap-6 text-xs text-muted-foreground">
        ...
        <span className="inline-flex items-center gap-1.5">
          <Zap className="h-3.5 w-3.5 text-primary" /> Web optimization profile
        </span>
        <span className="inline-flex items-center gap-1.5">
          <CheckCircle2 className="h-3.5 w-3.5 text-primary" /> Smaller PDF
        </span>
      </div>
    </>
  );
