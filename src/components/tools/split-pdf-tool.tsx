...
import { createPdfDownloadUrl, getPdfPageCount, revokeObjectUrl, splitPdf } from "@/lib/pdf-split";
...
function SplitPageThumbnail({ file, pageNumber }: { file: File; pageNumber: number }) {
...
export function SplitPdfTool() {
  ...
  const selectedPageSet = useMemo(
    () => new Set(selectedPagePreview.pages),
    [selectedPagePreview.pages],
  );
  ...
  const selectFile = async (nextFile: File | null) => {
    ...
    try {
      ...
      setProgressNote("Choose pages to split");
    } catch (err) {
      setFile(null);
      setStatus("error");
  ...
  const togglePage = (page: number) => {
    ...
    const currentPages = selectedPagePreview.error ? [] : selectedPagePreview.pages;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const bytes = await splitPdf(file, rangeInput);
    ...
    <>
      ...
      >
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Upload one PDF and choose the exact pages you want in the new file.
        </p>
      </label>
      ...
      {file && pageCount != null && (
        <div className="mt-4 rounded-2xl glass p-4 shadow-card">
          <div className="mb-3 text-sm font-medium">Pages to keep</div>
          <Input
            value={rangeInput}
            onChange={(event) => setRangeInput(event.target.value)}
            placeholder="Examples: 3, 1-5, 1-3,5,7-9"
            className="h-12 rounded-xl border-border bg-card/70 px-4 text-sm shadow-soft"
            disabled={status === "processing"}
          />
          <div className="mt-2 text-xs text-muted-foreground">
            Enter single pages or ranges from 1 to {pageCount}. Example: 1-3,5,7-9
          </div>
          <div className="mt-4 mb-2 flex flex-wrap items-center justify-between gap-2">
            <div className="text-sm font-medium">Page selection preview</div>
          ...
          >
            {visiblePageChips.map((page) => {
              ...
              return (
                ...
                >
                  {selected && (
                    <span className="absolute right-1 top-1 grid h-4 w-4 place-items-center rounded-full bg-primary-foreground text-primary shadow-soft">
                      <Check className="h-3 w-3" />
                    </span>
                  )}
                  {showThumbnails && file ? (
                    <>
                      <SplitPageThumbnail file={file} pageNumber={page} />
                      <span className="mt-1 leading-none">Page {page}</span>
                    </>
                  ) : (
          ...
          {selectedPagePreview.error ? (
            <div className="mt-3 rounded-xl border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive">
              {selectedPagePreview.error}
            </div>
          ) : selectedPagePreview.pages.length > 0 ? (
            <div className="mt-3 grid gap-1 text-xs text-muted-foreground sm:grid-cols-2">
              <div>
                Keeping pages:{" "}
                <span className="text-foreground">
                  {formatPageSummary(selectedPagePreview.pages)}
                </span>
              </div>
              <div>
                Output pages:{" "}
                <span className="text-foreground">{selectedPagePreview.pages.length}</span>
              </div>
            </div>
          ) : (
            <div className="mt-3 text-xs text-muted-foreground">
              Type a range or click page chips to choose pages.
            </div>
          )}
        </div>
      )}
      ...
      {status === "done" && downloadUrl && (
        <div className="mt-6 rounded-2xl glass p-6 text-center shadow-glow">
          ...
          <div className="font-semibold">Split complete</div>
          <p className="mt-1 text-sm text-muted-foreground">Your split PDF is ready to download.</p>
          <div className="mx-auto mt-4 grid max-w-md gap-2 rounded-2xl bg-card/70 p-4 text-left text-sm text-muted-foreground">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="font-medium text-foreground">{rangeInput}</span>
            </div>
            <div className="flex items-center justify-between gap-4">
      ...
      <div className="mt-6 flex items-center justify-center gap-6 text-xs text-muted-foreground">
        ...
        <span className="inline-flex items-center gap-1.5">
          <Scissors className="h-3.5 w-3.5 text-primary" /> Precise page ranges
        </span>
        <span className="inline-flex items-center gap-1.5">
          <Zap className="h-3.5 w-3.5 text-primary" /> Fast extraction
        </span>
      </div>
    </>
  );
